...
def upgrade() -> None:
    if not _table_exists("analog_digital_link"):
        op.create_table(
            ...
            sa.Column("digital_sensor_id", sa.Integer(), sa.ForeignKey("sensor.id"), nullable=False),
...
def downgrade() -> None:
    ...
    if not _column_exists("sensor", "is_system_wide"):
        op.execute(sa.text("ALTER TABLE sensor ADD COLUMN is_system_wide BOOLEAN NOT NULL DEFAULT 0"))
    ...
    if _table_exists("analog_digital_link"):
        ...
        for row in rows:
            ...
            conn.execute(sa.text(
                "UPDATE sensor SET parent_id=:p, is_system_wide=0 WHERE id=:d"
