...
#include "../core/dao/AppConfigDao.h"
...
// Minimal XOR-based obfuscation — replace with real AES/Fernet in production.
QString SettingsController::simpleEncrypt(const QString &plain) {
    return plain.toUtf8().toBase64();
}
QString SettingsController::simpleDecrypt(const QString &cipher) {
    auto decoded = QByteArray::fromBase64(cipher.toUtf8());
    return QString::fromUtf8(decoded);
}

QString SettingsController::ftpPassword() const {
    if (m_cfg.ftpPassword.isEmpty()) return {};
    return simpleDecrypt(m_cfg.ftpPassword);
}
void SettingsController::setFtpPassword(const QString &v) {
    m_cfg.ftpPassword = v.isEmpty() ? QString() : simpleEncrypt(v);
}
