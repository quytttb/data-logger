...
QT_DIR="${QT_DIR:-$HOME/Qt/6.12.1/gcc_64}"

if [[ ! -f "$QT_DIR/lib/cmake/Qt6/Qt6Config.cmake" ]]; then
    echo "Error: Qt not found at $QT_DIR" >&2
    echo "Set QT_DIR to your Qt kit, e.g. export QT_DIR=\$HOME/Qt/6.12.1/gcc_64" >&2
    exit 1
fi
...
if [[ ! -f "$QT_DIR/lib/cmake/Qt6HttpServer/Qt6HttpServerConfig.cmake" ]]; then
    ...
    echo "Open Qt Maintenance Tool → Add or remove components → Qt 6.12.1 → Additional Libraries → Qt HTTP Server" >&2
    exit 1
fi
