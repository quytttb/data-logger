...
private slots:
    void tableShape()
    {
        ...
        // VN kiosk default zone must be present and map to Asia/Ho_Chi_Minh.
        QCOMPARE(TimezoneOptions::indexOf(QStringLiteral("Asia/Ho_Chi_Minh")), 21);
    }
    ...
    void modelWithUnknownSystemPrependsEntry()
    {
        const QVariantList model =
            TimezoneOptions::modelWithSystem(QStringLiteral("Asia/Tokyo"));
        QCOMPARE(model.size(), 28);
