...
HistoryController: Quản lý truy vấn lịch sử (QThread) + xuất CSV (UTF-8 BOM).
"""
...
class HistoryController(QObject):
    """Điều khiển truy vấn lịch sử + xuất CSV."""
    ...
    def load_sensors(self):
        ...
        try:
            ...
            self._sensor_names = [self.tr("— All —")] + [s.name for s in sensors]
    ...
    def search(self, from_date: str, to_date: str, sensor_id: int = 0):
        ...
        try:
            ...
            self.messageSent.emit(
                self.tr("Error"),
                self.tr("Invalid date format. Use dd/MM/yyyy."),
            )
            return
        ...
        if from_dt > to_dt:
            self.messageSent.emit(
                self.tr("Error"),
                self.tr("Start date must be before end date."),
            )
            return
    ...
    def export_csv(self, path: str):
        ...
        try:
            ...
            self.messageSent.emit(
                self.tr("Success"),
                self.tr("Exported {0} row(s) to {1}.").format(len(items), path),
            )
            ...
            self.messageSent.emit(
                self.tr("Error"),
                self.tr("CSV export failed: {0}").format(e),
            )
    ...
    def _on_search_error(self, msg: str) -> None:
        ...
        self.messageSent.emit(
            self.tr("Error"),
            self.tr("Query failed: {0}").format(msg),
        )
