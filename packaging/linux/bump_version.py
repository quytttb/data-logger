#!/usr/bin/env python3
"""SemVer bump for project(DataLogger VERSION ...) in CMakeLists.txt."""
...
_VERSION_RE = re.compile(r"(?m)^(project\(DataLogger VERSION )([0-9.]+)")
...
def read_version() -> str:
    ...
    if not m:
        raise RuntimeError("project(DataLogger VERSION ...) not found in CMakeLists.txt")
    return m.group(2)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SemVer for DataLogger (CMakeLists.txt)")
