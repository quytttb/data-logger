                    ...
                    if found:
                        ...
                        for addr in range(0, 500, 10):
                            ...
                            if not res.isError() and not hasattr(res, 'exception_code'):
                                print(f"     -> [Tìm thấy dải Holding] Từ addr {addr}: {res.registers}")
                                # Tìm chi tiết
                        ...
                        for addr in range(0, 500, 10):
                            try:
                                ...
                                kwargs_input = {"slave": slave_id}
                            res = client.read_input_registers(address=addr, count=10, **kwargs_input)
                            if not res.isError() and not hasattr(res, 'exception_code'):
                                print(f"     -> [Tìm thấy dải Input] Từ addr {addr}: {res.registers}")
                                # Tìm chi tiết
                                for a in range(addr, addr+10):
                                    r = client.read_input_registers(address=a, count=1, **kwargs_input)
                                    if not r.isError() and not hasattr(r, 'exception_code'):
